...
def app():
    ...
    """
    page.render_header(
        title="Base64 Converter", caption="Convert files to Base64 and vice versa"
    )
    page.render_sidebar(title="Config")

    # Create main content
    object_type = page.sidebar.radio(
        label="Convert from", options=["File", "Text"], index=0
    )
    ...
    if object_type == "File":
        ...
        # Add file config
        group_all = page.sidebar.checkbox(
            "Group all files", value=True, help="Group all files into one zip file"
        )
        ...
        # Add file uploader
        uploaded_files = page.main.file_uploader(
        ...
        if uploaded_files:
            ...
            if button_1:
                ...
                # Show base64 text
                page.main.text_area(
                    label=new_filename, height=200, value=base64_data.decode("utf-8")
                )
